...
export const GameCard = async ({ game}: { game: GameDetails}) => {
    return (
      <Suspense fallback={<GameCardSkeleton />}>
        <section key={game.id} className='group overflow-hidden h-80 shadow-lg bg-card relative hover:scale-105 ease-in duration-150 rounded-lg border border-black-200'>
            ...
            <div className='bg-black w-full  absolute  bottom-0  text-white p-2 place-content-evenly'>
              <Link href={`/games/${game.id}`} className='flex flex-row justify-between'>
                  ...
                  <div className='flex flex-row justify-between mt-2 '>
                    ...
                    <a href={`/api/download/${game.id}`} className='hidden group-hover:block'><DownloadIcon/></a>
